...
@interface MSInvestStatusController ()
@property (assign, nonatomic) MSInvestStatusType type;
...
@implementation MSInvestStatusController
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [self pageEventWithTitle:@"投资成功" pageId:201 params:nil];
}
...
- (void)addSubViews {
    ...
    CGFloat btnBackY = 0;
    if (self.type == MSInvestStatusType_success) {
    ...
    CGFloat height = 20;
    CGFloat distance = 33;
    UIButton *btnAccount = [[UIButton alloc] initWithFrame:CGRectMake(self.view.width/2.0 - width - distance/2.0, CGRectGetMaxY(btnBack.frame)+27*scaleY, width, height)];
    [btnAccount setTitle:@"我的资产" forState:UIControlStateNormal];
    [btnAccount setTitleColor:[UIColor ms_colorWithHexString:@"#4229B3"] forState:UIControlStateNormal];
    btnAccount.titleLabel.font = [UIFont systemFontOfSize:14];
    [[btnAccount rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
        @strongify(self);
        [self popToIndex:2];
    }];
    ...
    UIButton *btnInvest = [[UIButton alloc] initWithFrame:CGRectMake(self.view.width/2.0 + distance/2.0, CGRectGetMaxY(btnBack.frame)+27*scaleY, width, height)];
    [btnInvest setTitle:@"继续投资" forState:UIControlStateNormal];
    btnInvest.titleLabel.font = [UIFont systemFontOfSize:14];
    [btnInvest setTitleColor:[UIColor ms_colorWithHexString:@"#4229B3"] forState:UIControlStateNormal];
    [[btnInvest rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
        @strongify(self);
        [self popToIndex:1];
    }];
    [self.scrollView addSubview:btnInvest];
    
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake((self.view.width - 1)/2.0, 0, 1, 8)];
    line.centerY = btnAccount.centerY;
    line.backgroundColor = [UIColor ms_colorWithHexString:@"#999999"];
    ...
    self.scrollView.contentSize = CGSizeMake(0, CGRectGetMaxY(btnInvest.frame));
